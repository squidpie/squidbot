...
class PluginLoaderTest : public testing::Test {
  protected:
    ...
    void SetUp() override {
      context = std::make_shared<CoreContext>(plog::get(), server, service_manager, plugin_manager, TEST_LIB_DIR);
    }
};
...
TEST_F(PluginLoaderTest, setup) {
  ...
  EXPECT_CALL(*plugin_manager, _register_plugin(testing::_, testing::_)).WillOnce(testing::Return());
