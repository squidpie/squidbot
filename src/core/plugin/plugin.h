...
typedef std::unordered_map<std::type_index, std::shared_ptr<ServiceInterfaceBase>> InterfaceMap_t;
...
public:
  ...
  PluginRunAction(std::shared_ptr<PluginRunActionContext> context)
      : client(context->client), filter(context->filter), action(context->action) {}
  ~PluginRunAction() {}
...
public:
  ...
  Plugin(std::shared_ptr<CoreContext> context) {
    auto client = configure_client(context);
    configure_plugin(client, context);
  ...
  void inject(std::shared_ptr<RunnerBase> _runner) { runner = _runner; }
#endif
...
protected:
  ...
  auto configure_client(std::shared_ptr<CoreContext> context) {
    auto client = context->event_server->create_client();
    client->subscribe(TEST_EVENT_TYPE);
    return client;
  }

  void configure_plugin(auto client, auto context) {
